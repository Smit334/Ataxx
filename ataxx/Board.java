...
class Board {
    ...
    void clear() {
        ...
        for (int i = 2; i < 9; i++) {
            for (int j = 2; j < 9; j++) {
                _board[(EXTENDED_SIDE * i) + j] = EMPTY;
            }
        }
        unrecordedSet(EXTENDED_SIDE * 3 - 3, RED);
        unrecordedSet(EXTENDED_SIDE * 9 - 9, RED);
        _numPieces[RED.ordinal()] = 2;
        unrecordedSet(EXTENDED_SIDE * 2 + 2, BLUE);
        unrecordedSet(EXTENDED_SIDE * 8 + 8, BLUE);
    ...
    void makeMove(Move move) {
        ...
        for (int i: aroundMe(move.toIndex())) {
            _numPieces[_whoseMove.ordinal()]++;
        ...
        if (_numJumps == JUMP_LIMIT || _totalOpen == 0
            ...
            _winner = (numRed == numBlue) ? EMPTY
                    : ((numRed > numBlue) ? RED : BLUE);
        } else if (numBlue == 0) {
            _winner = RED;
        } else if (numRed == 0) {
            _winner = BLUE;
    ...
    /** Gives me a list of opponents around me.
     * @return null
     * @param index */
    ArrayList<Integer> aroundMe(int index) {
        ArrayList<Integer> opponentsAroundMe = new ArrayList<Integer>();
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                int box = neighbor(index, i, j);
                if (_board[box] == _whoseMove.opposite()) {
                    opponentsAroundMe.add(box);
                }
            }
        }
        return opponentsAroundMe;
    }
    ...
    void undo() {
        while (_undoSquares.peek() != null) {
            _numPieces[_undoPieces.peek().ordinal()]++;
            _numPieces[get(_undoSquares.peek()).ordinal()]--;
            unrecordedSet(_undoSquares.pop(), _undoPieces.pop());
        }
        _undoSquares.pop();
        _whoseMove = _whoseMove.opposite();
        _numJumps = (_allMoves.get(_allMoves.size() - 1).isJump())
                ? _numJumps - 1 : _numJumps;
        _totalOpen = (_allMoves.get(_allMoves.size() - 1).isExtend())
                ? _totalOpen + 1 : _totalOpen;
    ...
    boolean legalBlock(char c, char r) {
        return get(c, r) == EMPTY
                && get(reflectHorizontal(c), r) == EMPTY
                && get(c, reflectVertical(r)) == EMPTY
                && get(reflectHorizontal(c), reflectVertical(r)) == EMPTY
                && _allMoves.isEmpty();
    }
    ...
    void setBlock(char c, char r) {
        ...
        set(c, r, BLOCKED);
        set(c, reflectVertical(r), BLOCKED);
        set(reflectHorizontal(c), r, BLOCKED);
        set(reflectHorizontal(c), reflectVertical(r), BLOCKED);
    ...
    /** Returns the vertically reflected r-co-ordinate.
     * @param r */
    char reflectVertical(char r) {
        return (char) ((int) '4' + '4' - r);
    }

    /** Returns the vertically reflected c-co-ordinate.
     * @param c */
    char reflectHorizontal(char c) {
        return (char) ((int) 'd' + 'd' - c);
    }
    ...
    /** A function to get the one dimensional board Array.
     * @return PieceColor[] */
    public PieceColor[] get1DBoard() {
        return _board;
    }
